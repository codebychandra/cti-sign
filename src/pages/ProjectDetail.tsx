...
function TemplateTab({ template, ensureTemplate, renameTemplate, deleteTemplate, fieldsManagerProps }: { projectId: string; template?: Form; customFields: ProjectCustomField[]; ensureTemplate: () => void; renameTemplate: (templateId: string, name: string) => void; deleteTemplate: (templateId: string) => void; fieldsManagerProps: SettingTabProps }) {
  ...
  return <div className="space-y-8"><section className="space-y-4"><div><h2 className="font-heading text-lg font-bold text-cti-black">Template PDF</h2><p className="mt-1 text-sm text-cti-gray">Upload the PDF and map fields onto it. Define input fields below first so they're available to map.</p></div><div className="card space-y-4 p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0 flex-1">{editing && template ? <div className="flex gap-2"><input className="input" value={name} onChange={(e) => setName(e.target.value)} /><button className="btn-primary" onClick={() => { renameTemplate(template.id, name); setEditing(false) }}>Save</button><button className="btn-ghost" onClick={() => { setName(template.name); setEditing(false) }}>Cancel</button></div> : <><p className="font-semibold text-cti-ink">{template?.name ?? 'Template'}</p><p className="text-sm text-cti-gray">{template?.has_template ? `Template Ready - ${template.page_count} Page(s)` : 'No PDF Uploaded Yet'}</p></>}</div>{template && <div className="flex shrink-0 items-center gap-1.5"><IconButton label="Edit Name" size="md" onClick={() => setEditing(true)}><PencilIcon /></IconButton><Link to={`/forms/${template.id}/edit`} title="Replace PDF" aria-label="Replace PDF" className="grid h-9 w-9 shrink-0 place-items-center rounded-md border border-cti-line text-cti-ink transition-colors hover:bg-cti-bg"><UploadIcon /></Link><IconButton label="Delete Template" tone="danger" size="md" onClick={() => deleteTemplate(template.id)}><TrashIcon /></IconButton></div>}{template ? <Link to={`/forms/${template.id}/edit`} className="btn-primary whitespace-nowrap">Upload & Mapping PDF</Link> : <button className="btn-primary whitespace-nowrap" onClick={ensureTemplate}>Upload & Mapping PDF</button>}</div></div></section><SettingTab {...fieldsManagerProps} /></div>
}
...
function IconButton({ label, onClick, disabled, tone, size = 'sm', children }: { label: string; onClick: () => void; disabled?: boolean; tone?: 'danger'; size?: 'sm' | 'md'; children: React.ReactNode }) {
  const toneClass = tone === 'danger' ? 'text-cti-red hover:bg-red-50' : 'text-cti-ink'
  const sizeClass = size === 'md' ? 'h-9 w-9' : 'h-8 w-8'
  return <button type="button" title={label} aria-label={label} disabled={disabled} onClick={onClick} className={`grid ${sizeClass} shrink-0 place-items-center rounded-md border border-cti-line transition-colors hover:bg-cti-bg disabled:cursor-not-allowed disabled:opacity-30 ${toneClass}`}>{children}</button>
}
